...
function updateScrollbarFromChart(chart) {
  ...
  const widthPct = Math.max(
    2,
    Math.min(100, ((viewMax - viewMin) / range) * 100),
  );
...
function setChartViewport(viewMin, viewMax) {
  ...
  if (viewMin < chartFullMin) {
    viewMin = chartFullMin;
    viewMax = viewMin + viewRange;
  }
  if (viewMax > chartFullMax) {
    viewMin = chartFullMax - viewRange;
    viewMax = chartFullMax;
  }
...
function initScrollbar() {
  ...
  function onTrackClick(e) {
    ...
    const targetLeftPct = Math.max(
      0,
      Math.min(100 - thumbWidthPct, clickPct - thumbWidthPct / 2),
    );
  ...
  function onDragMove(e) {
    ...
    const newLeft = Math.max(
      0,
      Math.min(100 - thumbWidthPct, startLeftPct + dxPct),
    );
  ...
  historyChart = new Chart(ctx, {
    ...
    options: {
      ...
      onClick: async (event, elements) => {
        ...
        const point = historyData.find(
          (d) => Math.abs(d.timestamp - tsSec) < 0.5,
        );
        if (point) {
          pinnedPoint = point;
          await showPinnedPlayers(point);
        }
      },
    },
    plugins: plugins,
...
async function loadHistoryChart(sid, range) {
  document
    .querySelectorAll("#rangeBtns .btn-outline")
    .forEach((b) => b.classList.remove("active-range"));
  const activeBtn = document.querySelector(
    `#rangeBtns [data-range="${range}"]`,
  );
  ...
  const showDate =
    ["24h", "7d", "30d"].includes(range) ||
    (range === "custom" && totalDuration > 86400);
...
async function appendRealtimeData(sid) {
  ...
  historyChart.options.scales.y.max = computeYMax(
    historyData.map((d) => d.player_count || (d.online ? 0 : null)),
  );
...
async function showPinnedPlayers(point) {
  ...
  document.getElementById("pinnedTime").textContent = new Date(
  ...
  if (list.length === 0 && detailServerId) {
    const resp = await fetch(
      `/api/servers/${detailServerId}/player-list?ts=${point.timestamp}`,
    );
  ...
  if (list.length === 0) {
    plEl.innerHTML =
      '<span style="color:var(--muted);font-size:0.85rem;">该时间点无玩家在线</span>';
  } else {
    let anonCount = 0;
    const normalNames = [];
