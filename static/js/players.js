...
function renderPlayerRow(p) {
  const displayName =
    p.anon_count > 0 ? `${esc(p.name)} x${p.anon_count}` : esc(p.name);
  return `
...
async function loadPlayerList() {
  try {
    ...
    if (playerSearchTerm) {
      anonPlayers = anonPlayers.filter((p) =>
        p.name.toLowerCase().includes(playerSearchTerm),
      );
      normalPlayers = normalPlayers.filter((p) =>
        p.name.toLowerCase().includes(playerSearchTerm),
      );
    }
    ...
    if (anonPlayers.length > 0) {
      ...
      anonList.innerHTML = playerSearchTerm
        ? ""
...
document.querySelectorAll("#page-players .btn-outline").forEach((btn) => {
  btn.addEventListener("click", () => {
    document
      .querySelectorAll("#page-players .btn-outline")
...
function _refreshPlayerDetailState(statuses) {
  ...
  for (const s of statuses) {
    ...
    if (match) {
      found = { server_name: s.server_name, online: s.online };
      break;
    }
  }
...
function openPlayerDetail(name) {
  ...
  playerDetailPrevPage =
    currentPage !== "player-detail" ? currentPage : playerDetailPrevPage;
    ...
    if (servers.length === 0) {
      ...
      rsHtml = servers
        .map((s, i) => {
          ...
          const logout = s.logout_time
            ? new Date(s.logout_time * 1000).toLocaleString("zh-CN")
            : "至今";
