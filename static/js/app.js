...
function setTheme(theme) {
  ...
  themeIcon
    ...
    .setAttribute(
      "href",
      theme === "light" ? "#icon-theme-dark" : "#icon-theme-light",
    );
...
function _requireLogin(page) {
  if (loggedIn || !_requireLoginEnabled) {
...
function switchPageDirect(page) {
  ...
  if (page === "detail" && detailServerId) {
    ...
    page === "player-detail" &&
    typeof loadPlayerDetail === "function" &&
    playerDetailName
  ) {
...
function renderHome() {
  ...
  grid.innerHTML = currentStatuses
    .map((s) => _renderServerCard(s, false))
    .join("");
}
...
function renderServers() {
  const total = currentStatuses.length;
  document.getElementById("serversEmpty").style.display =
    total === 0 ? "block" : "none";
  ...
  grid.innerHTML = currentStatuses
    .map((s) => _renderServerCard(s, true))
    .join("");
}
...
function openDetail(sid, range, startTs, endTs) {
  ...
  if (range === "custom" && startTs && endTs) {
    ...
    document.getElementById("rangeStartDate").value = sDate
      .toISOString()
      .slice(0, 10);
    document.getElementById("rangeStartTime").value = sDate
      .toTimeString()
      .slice(0, 5);
    document.getElementById("rangeEndDate").value = eDate
      .toISOString()
      .slice(0, 10);
    document.getElementById("rangeEndTime").value = eDate
      .toTimeString()
      .slice(0, 5);
...
function loadDetailPage(sid) {
  ...
  _setTextSafe(
    document.getElementById("dLatency"),
    s.latency != null ? s.latency + " ms" : "--",
  );
  _setTextSafe(
    document.getElementById("dPlayers"),
    s.players.online + " / " + s.players.max,
  );
  document.getElementById("dMotd").innerHTML =
    s.motd_html || esc(s.motd) || "--";
  ...
  if (s.server_type === "bedrock" && s.online) {
    ...
    _setTextSafe(
      document.getElementById("dBedrockGamemode"),
      s.gamemode || "--",
    );
  ...
  const anonCount = playerList.filter((p) =>
    (p.name || "").includes(" "),
  ).length;
...
function getBackups() {
  return Array.from(document.querySelectorAll("#backupList .backup-row"))
    .map((r) => r.querySelector(".bkp-addr").value.trim())
    .filter((v) => v);
}
...
document.getElementById("saveServer").addEventListener("click", () => {
  ...
  fetch(`/api/servers/check-name?name=${encodeURIComponent(name)}`)
    .then((r) => r.json())
    .then((res) => {
      if (!res) {
        _doSaveServer(isEdit, data);
        ...
        document.getElementById("dupNameMsg").textContent =
          `服务器 "${name}" 之前被删除，仍有 ${res.sessions} 条玩家在线记录残留。`;
...
function resetForm() {
  ...
  document.getElementById("saveServer").innerHTML =
    '<svg class="svg-icon sm"><use href="#icon-check"/></svg> 保存服务器';
}
...
function editServer(sid) {
  fetch(`/api/servers/${sid}`)
    ...
    .then((s) => {
      ...
      document.getElementById("saveServer").innerHTML =
        '<svg class="svg-icon sm"><use href="#icon-check"/></svg> 更新服务器';

      const rconHost = s.rcon_host || "";
      const rconPort = s.rcon_port || "";
      document.getElementById("srvRconHost").value = rconHost
        ? rconPort
          ? rconHost + ":" + rconPort
          : rconHost
        : "";
...
function confirmDeleteServer(sid) {
  ...
  hideDeleteConfirm();
  fetch(`/api/servers/${sid}?clean_data=${cleanData ? "1" : "0"}`, {
    method: "DELETE",
  })
    .then(() => renderAdmin())
    .catch(() => {});
...
document.getElementById("saveSettings").addEventListener("click", () => {
  const check_interval =
    parseInt(document.getElementById("intervalInput").value) || 5;
  ...
  const db_path =
    document.getElementById("dbPath").value.trim() || "monitor.db";
  const offline_threshold =
    parseInt(document.getElementById("offlineThreshold").value) || 2;
...
function deleteAdminPlayer(name) {
  ...
  fetch(`/api/players/${encodeURIComponent(name)}`, { method: "DELETE" })
    .then(() => {
...
let playerNotifServers = JSON.parse(
  localStorage.getItem("playerNotifServers") || "{}",
); // { server_id: true }
let prevStatuses = [];
...
function updateServerNotifBtn() {
  ...
  if (btn) {
    ...
    btn.className =
      "btn btn-sm " + (serverNotifEnabled ? "btn-primary" : "btn-outline");
  }
}
  ...
  if (playerNotifServers[sid]) {
    ...
    _ensureNotifPermission(() => {
      ...
      localStorage.setItem(
        "playerNotifServers",
        JSON.stringify(playerNotifServers),
      );
      updatePlayerNotifBtn(sid);
    });
  }
...
function checkNotifications(newData) {
  if (_skipNotificationCheck) {
    _skipNotificationCheck = false;
    return;
  }
  ...
  for (const s of newData) {
    ...
    if (serverNotifEnabled) {
      ...
      if (!s.online) {
        if (prev.online) _offlineCounts[s.server_id] = 1;
          ...
          _offlineCounts[s.server_id] = (_offlineCounts[s.server_id] || 0) + 1;
        if (_offlineCounts[s.server_id] === _offlineThreshold) {
          _offlineNotified[s.server_id] = true;
          _notify(
            s.server_name,
            `服务器已离线（连续 ${_offlineThreshold} 次）`,
          );
        }
      }
    }
    ...
    if (
      playerNotifServers[s.server_id] &&
      s.online &&
      prev.online &&
      (s.has_rcon || s.players.online <= 12)
    ) {
      const prevNames = new Set((prev.players.list || []).map((p) => p.name));
      const currNames = new Set((s.players.list || []).map((p) => p.name));
      for (const n of currNames) {
        if (!prevNames.has(n) && !n.includes(" "))
          _notify(s.server_name, `${n} 加入了`);
      }
      for (const n of prevNames) {
        if (!currNames.has(n) && !n.includes(" "))
          _notify(s.server_name, `${n} 离开了`);
      }
    }
  }
}
...
socket.on("status_update", (data) => {
  ...
  if (currentPage === "player-detail" && playerDetailName)
    _refreshPlayerDetailState(data);
});
...
async function executeOptimize() {
  ...
  try {
    ...
    const deleteDays =
      parseInt(document.getElementById("optDeleteDays").value) || 0;
