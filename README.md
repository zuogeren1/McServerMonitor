...
当前分支使用 React + shadcn/ui 重构了前端，首次运行需先构建：
...
`app.py` 会自动检测 `frontend/dist/` 目录并服务 Vite 构建产物；未检测到时回退至旧版 HTML 模板。
...
cd frontend
npm install
npm run dev     # Vite HMR 开发服务器，代理 API 到 Flask :9000
```
...
在服务器详情页，支持 15 分钟（实时）/ 1h / 6h / 24h / 7d / 30d / 自定义时间段查询。除实时外均支持缩放，建议大跨度时间段使用拖拽框选而非滚轮（避免快速缩放时卡顿）。点击数据点可固定显示该时刻的在线玩家列表。
...
├── frontend/                   React 前端（当前分支）★
│   ├── index.html              Vite 入口 HTML
│   ├── vite.config.ts          Vite 配置（代理 /api → Flask）
│   └── src/
│       ├── main.tsx            React 入口，Chart.js 组件注册
│       ├── App.tsx             根组件（布局、页面路由、通知、favicon）
│       ├── index.css           Tailwind CSS + CSS 变量主题
│       ├── lib/                工具层：utils, api（fetch 封装）, socket（模块级单例）
│       ├── store/              Zustand 状态：server, player, auth, ui
│       ├── hooks/              useSocket, useTheme, useNotification, useMediaQuery
│       ├── components/
│       │   ├── layout/         Sidebar, MobileMenu, ServerNotifButton
│       │   ├── ui/             shadcn/ui 组件（Button, Card, Dialog 等）
│       │   ├── home/           StatsBar, ServerCard, HomePage, ServersPage
│       │   ├── detail/         ServerInfoPanel, BackupStatusPanel, HistoryChart, ChartScrollbar, RangeSelector
│       │   ├── players/        PlayersPage, PlayerDetailPage
│       │   ├── admin/          AdminPage, PlayerManagePage
│       │   └── modals/         LoginDialog
│       └── plugins/
│           └── chart/          Chart.js 插件：crosshair, scrollbarSync, realtimeAppend
├── templates/                  （旧版模板，未检测到 frontend/dist 时回退使用）
└── static/                     （旧版静态文件）
```
...
服务器可选填 RCON 地址和密码，不填不影响正常使用。启用后，每次查询额外通过 RCON `list` 命令获取完整玩家列表（mcstatus 查询仅返回至多 12 人的样本），用于更准确的玩家在线追踪。配置了RCON的服务器上玩家通知不受"在线人数 ≤ 12"的限制。
...
- shadcn/ui（Button, Card, Dialog, Input, Badge, Select, Checkbox, Tooltip）
- Tailwind CSS v4（CSS 变量主题，深色/浅色切换）
- Zustand 状态管理（4 stores：server / player / auth / ui，数据层与 UI 层分离）
- Chart.js + chartjs-plugin-zoom（手动管理实例，十字线插件，滚动条同步，LTTB 抽稀）
- Socket.IO 客户端
- Vite 构建
